class Net {
  ...
  getPionki() {
    console.log("start");
    setInterval(() => {
      fetch("/compareTable", { method: "post" }).then((resp) =>
        resp.json().then((data) => {
          if ((data.info = "ok")) {
            const { xstart, zstart, xend, zend, move } = data;
            this.game.setPionki(xstart, zstart, xend, zend, move);
          }
        })
      );
    }, 1000);
  }
  Move(xs, zs, xe, ze, user) {
    console.log("move");
    const body = JSON.stringify({ xs, zs, xe, ze, user });
    const headers = { "Content-Type": "application/json" };
